...
> **Los encargos ya no viven aquí.** Desde el **2026-08-20**, por regla del Jefe,
> Doc reparte en `PROMPT_GRAVITY.md` — un archivo **local a tu terminal**, que
> está en `.gitignore` y no viaja a git.
>
> **Este archivo es tuyo y es la evidencia de lo que hiciste.** Mezclarlo con
> órdenes ajenas hace que se pierda el hilo: no se distingue lo que se pidió de
> lo que se entregó, y quien lo lea dentro de tres meses no sabrá cuál de las dos
> cosas está leyendo. Doc no lo edita. Escribe aquí lo hecho, con su commit.

## Lo último entregado

| Encargo | Dónde quedó |
| --- | --- |
| Desbloqueo de CI: causa en re-lanzamiento de error (`preserve-caught-error`) y detección tipada de 409 con `ApiError.status` | `d2ae401` — Añadido `{ cause: err }` en `tasks.api.ts:102`; corregido `ChangeEmailStatusCard.tsx` para comprobar `err instanceof ApiError && err.status === 409` en lugar de `message.includes`; ajustada aserción de modelo en `copilot.spec.ts`. Monorepo lint y suite completa de tests en verde (33 suites pasadas, 0 errores de ESLint). |
| §44.5, §44.8, §44.9: Renderizado condicional `aiConfidence`, limpieza de tipos muertos y traducción UI | `6b1f1b0` — Corregido `{typeof task.aiConfidence === 'number' && <AiAuditBadge />}` para no pintar `0` suelto en falsy; eliminada interfaz `TriageEmail` en desuso; unificada importación de `TaskPriority` desde `../types`; traducidos `▶ Iniciar` y `⏹ Detener`. |
| §44.1: Soporte de tarjeta y acción para `change_email_status` en Copilot | `bc860e3` — Creado `ChangeEmailStatusCard.tsx`, integrado en `ChatMessage.tsx` y procesador SSE de `CopilotDrawer.tsx` con manejo de confirmación/descarte, soporte de `force` para 409 y sin botón cuando `status: null`. |
| §43.1: Mitad cliente de reconexión de socket y migración de las 21 llamadas `fetch` a `apiFetch` | `7408546` — Implementado listener de `SESSION_EVENTS.rechazada` y `connect_error` distinguiendo `CADUCADA` (refresco + reconexión) de `INVALIDA` (redirige a login) con tope de 5 reintentos; migradas todas las 21 llamadas de `tasks.api`, `time.api`, `tags.api` y `copilot.api` a `apiFetch` protegido por el cerrojo de refresco. |
| Exposición pública de `/version.json` con SHA de commit, fecha y `no-cache` | `6f63060` — Plugin en `vite.config.ts` emite `version.json` con `VERCEL_GIT_COMMIT_SHA` y timestamp UTC sin requerir sesión; cabecera `Cache-Control: no-cache, no-store, must-revalidate` configurada en `vercel.json`. |
| Resiliencia de `ignoreCommand` en Vercel ante clones superficiales (`git cat-file`) | `f4afa28` — Añadida comprobación `git cat-file -e` sobre `$VERCEL_GIT_PREVIOUS_SHA` con `exit 1` (construir por defecto) si el commit previo no existe en el shallow clone o está vacío, desbloqueando el despliegue del frontend. |
