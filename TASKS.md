...
- [ ] 🟡 Realtime: `socket.io` emite `task.*` → UI se actualiza sin recargar — ✅ Backend completo: `TasksGateway` emite `task.created`, `task.updated` (desde `PATCH`, desde el arrastre y desde el barrido horario) y `task.deleted`. Todos los payloads llevan `userId`. _Faltan: **salas por usuario** y autenticación del handshake —hoy el broadcast es global y sin autenticar—, el consumo en el frontend, y difundir la renumeración de hermanas tras un arrastre_
