...
- [ ] 🔴 Bandeja: botones de estado y pestañas conectadas al endpoint (Gravity) — _la UI está maquetada desde antes; solo falta enchufarla a `PATCH /emails/:id/status` y filtrar por `?status=`_
- [ ] 🟡 Que la bandeja se entere en vivo de los cambios de estado — _hoy `PATCH` no emite socket: si el mismo usuario tiene dos pestañas, la otra no se entera hasta recargar. El gateway ya existe (`TasksGateway`); haría falta un evento `email.updated`_
