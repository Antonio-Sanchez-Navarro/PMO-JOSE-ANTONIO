...
- [ ] 🟡 Bandeja: botón para devolver un correo a pendientes (Gravity) — _no existe; los tres botones de la fila solo mandan `DISMISSED`, `IN_PROGRESS` y `COMPLETED`, así que desde el navegador no hay forma de reabrir un correo. Tiene que mandar `{ "status": "PENDING", "force": true }`_
