...
- [ ] 🟡 Realtime: `socket.io` emite `task.*` → UI se actualiza sin recargar — ✅ Backend completo: `TasksGateway` emite `task.created`, `task.updated` (desde `PATCH`, el arrastre y el barrido horario), `task.reordered` (orden de las columnas tras un arrastre) y `task.deleted`. El handshake se autentica con la cookie `pmo_session` y cada cliente entra en la sala de su `userId`, así que los eventos no salen de su dueño. _Falta el consumo en el frontend_
